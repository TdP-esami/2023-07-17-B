...
public class Model {
	...
	public void creaGrafo(Integer anno, String brand) {
		this.grafo = new SimpleWeightedGraph<Products, DefaultWeightedEdge>(DefaultWeightedEdge.class);
		
		//Vertici
		List<Products> vertici = this.dao.getVertici(brand);
		Graphs.addAllVertices(this.grafo, vertici);
		
		
		//Archi
		for (int i =0; i < vertici.size(); i++) {
			Products v_i = vertici.get(i);
			for (int j =i+1; j < vertici.size(); j++) {
				Products v_j = vertici.get(j);			
				
				int peso = this.dao.getPeso(v_i, v_j, anno);
				if (peso>0) {
					Graphs.addEdgeWithVertices(this.grafo, v_i, v_j, peso);
				}
			}
		}
